...
class fenetre(QWidget) :
    ...
    def openFileDialog(self):
        ...
        for index in range(len(listeFichiers)):
            image = np.array_split(fits.getdata(get_pkg_data_filename(fname+"/"+listeFichiers[index]),ext=0))
            print(image)
            for x in range(len(image)):
                rouge.append[image[x][0]]
                vert.append(image[x][1])
                bleu.append(image[x][2])
        new_image = []
        new_image.append(rouge)
        new_image.append(vert)
        new_image.append(bleu)
        ...
        for i in range(len(new_image)):
            ...
            for j in range(len(new_image[i])):
                # calcule le z-score
                z = (new_image[i][j]-np.mean(new_image[i]))/np.std(new_image[i])
                # si thresold est inférieur au z-score alors supprime le pixel incohérents
                if (z > threshold).any():
                    new_image[i].remove(new_image[i][j])
                    
        self.listeFichiers = new_image
